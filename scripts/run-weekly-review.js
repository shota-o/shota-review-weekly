...
- *アスタリスク太字*禁止。**ダブル**も禁止。## # も禁止。
- セクション見出しは :emoji: テキスト の形式。
- 絵文字はSlackショートコードで書く（:sunny: :calendar: :email: :speech_balloon: :chart_with_upwards_trend: :wrench: :bulb: :clock3:）
...
レイアウトの重要ルール：
- 各日のまとまりを > 引用ブロックで囲むこと（全行を > で始める）
- 日と日の間は空行を入れて余白をつくること
- 曜日ラベル（月 3/10 等）は引用ブロックの外に置く
- 会社名・キーワードは \`バッククォート\` で囲む
- コードブロック（\`\`\`）はメインコンテンツには使わない（文字が小さくなるため）

:sunny: 今週の振り返り（${thisWeekLabel}）

出力フォーマット（この通りに書くこと。> で囲んでグループ化）：

月 3/10
> \`PwC\` と○○の件で打合せ → 方向性で合意
> \`アステラス製薬\` にNoLang紹介 → 関心高め、次回デモ設定の流れ
...
水 3/12
> \`鈴鹿市\` \`奥州市\` \`西尾市\` の3自治体と商談
> いずれもDX推進の文脈。自治体向けの横展開が順調

各日2-4行。何もなかった日は飛ばす。
全行を > で始めること。日と日の間に必ず空行を入れること。
ポイントは「何が進んだか」「何が決まったか」「宿題は何か」。

:calendar: 来週やること整理（${nextWeekLabel}）

同じフォーマットで出力。引用ブロックで日ごとにグループ化。

月 3/17
> \`BEMAC + PwC\` 対面 → 前回の宿題を持っていく。資料は日曜中に確認
> \`川口市\` 齋藤様（初回）→ 自治体DXの文脈。自治体向け資料を準備
> :clock3: \`PR TIMES\` 修正版は今日中に送る必要あり

火 3/18
> \`デンソー\` と \`レスター\` が時間被り → デンソー優先でレスターはリスケ提案
> \`大広wedo\` 対面（東京本社）→ 前回宿題の○○を持参

各日のポイント：
- その日で一番大事なことは何か
- 準備すべきことと背景
- デッドライン（ :clock3: マーク）
- 重複があればどちらを優先すべきか`, 8000);
  sections.push(sec1);
...
レイアウトの重要ルール：
- 各メールの情報を > 引用ブロックで囲んでグループ化する
- メールとメールの間は空行を入れて余白をつくる
- 件名は \`バッククォート\` で囲む
- 各メールの説明は1-2行の自然な文章。冗長に書かない
- コードブロックは使わない
...
出力フォーマット（この通りに書くこと。各メールを > で囲む）：

> \`見積依頼の件\` ← ○○社 △△さん（3/12）
> 導入検討中で□□プランの見積依頼。資料を添えて今週中に返信

> \`提携の方向性について\` ← ××社 ▽▽さん（3/13）
> 具体的条件提示あり。来週打合せ前に社内で方針固めておく
...
> \`お打合せのお礼\` → □□社 ▲▲様（送信 3/12）
> 商談後のお礼。好感触だったので金曜にもう一度フォロー

→ もう一度連絡した方がいい相手

> ○○社 △△さん
> 前回から2週間経過。検討中だった□□の件、進捗確認のメールを`, 8000);
  sections.push(sec2);
...
レイアウトの重要ルール：
- 各項目を > 引用ブロックで囲んでグループ化
- 項目と項目の間は空行を入れて余白をつくる
- トピック名・会社名は \`バッククォート\` で囲む
- コードブロックは使わない

:speech_balloon: Slackから拾っておくこと

翔太が見落としていそうなもの・覚えておくべきことを幅広く。

出力フォーマット（各項目を > で囲む。項目間に空行）：
...
> Notionのタスクで気にしておくべきもの、期限が近いもの、止まっているものを引用ブロックで項目別に整理
